...
class FractureSequence(keras.utils.Sequence):
    ...
    def __init__(self, x_infos, y_infos, batch_size, crop_shape, shuffle=True):
        '''
        x_paths: list of paths to preprocessed images
        '''
...
def get_nifti_fracture_datagens(preprocessed_metadata_path, batch_size, crop_shape, seed=None, validation_split=0.25):
    '''
    ...
    normal_shuffled = normal_infos.sample(frac=1, random_state=seed)
    ...
    print('Batch size:', batch_size)
    train_gen = FractureSequence(train_x, train_y, batch_size, crop_shape)
    val_gen = FractureSequence(val_x, val_y, batch_size, crop_shape)
    print('Num train batches:', len(train_gen))
    print('Num val batches:', len(val_gen))
    return train_gen, val_gen
